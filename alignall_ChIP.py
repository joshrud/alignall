...
# 200420 | Copied everything from ATAC pipeline into this script and find/replaced all mentions of ATAC with ChIP
#
#
#
#
...
###### plug in for performing ChIP-seq peak calling in alignall 
...
def getConditionGroups(alignmentDir):
	...
	splitNames = {} #example: {0: ['ChIP'], 1: ['CP101', 'JD130'], 2: ['Q111', 'Q20'], 3: ['1', '2', '3']} #for more complex conditions
	for s in allSamples:
...
def runCallPeaks(): 
	...
	func = partial(exMergedMacs, macsGenome) #don't need the inputs for ChIP 
...
def exChipHTseq(htSeqStrand, gtf, j):
	subprocess.call("htseq-count -f bam -r pos -s " + htSeqStrand + " -t peak -i gene_id -m intersection-nonempty  "+alignmentDir++j+"/Aligned.sortedByCoord.duplicateRemoved.out.bam " + gtf + " > "+alignmentDir++j+"/ChIP.duplicateRemoved.counts.txt 2>  "+alignmentDir++j+"/ChIP.HTseq.duplicateRemoved.log.txt", shell=True)

def ChipCounts():
	
	newfile4=open("counts.ChIP."+genome+".txt",'w')
	d=OrderedDict()
	samples=[]
	...
	for i in os.listdir(alignmentDir):
		...
		if i.endswith("out"):
			name=i.strip('out')
			HTseqFile = ""+alignmentDir++i+"/ChIP.duplicateRemoved.counts.txt"
